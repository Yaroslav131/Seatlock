...
## Лимит частоты gateway

С этапа 3 gateway ограничивает запросы по IP (600 в минуту на обычные пути, 60 на занятие места,
30 на создание заказа, 20 на вход; счётчики в Redis). Нагрузочный тест с одного адреса упрётся в
это за секунды и получит 429 вместо реальных цифр. Поэтому:

- **локально:** запустите dev-gateway с `RATE_LIMIT_ENABLED=false` (в `.env`);
- **на проде:** добавьте свой внешний IP в `RATE_LIMIT_BYPASS_IPS=<ip>` в `.env` на сервере
  и перезапустите gateway (`docker compose -f docker-compose.prod.yml up -d --no-deps gateway gateway-2`
  по одной реплике). **После теста верните значение пустым**, иначе лимит для этого адреса так и
  останется отключённым.

Пока лимит включён, 429 в отчёте k6 — это его срабатывание, а не деградация системы; смотрите
`gateway_rate_limited_total` в Prometheus.

Про посев покупателей: с этапа 2 `notification` берёт адрес из снимка в `order.paid` (его кладёт `payment` из
JWT покупателя, ADR 0005), а не из `auth`. Поэтому `seed-buyers.sql` для прохождения цепочки больше не нужен:
`BUYER_POOL` по-прежнему задаёт число разных покупателей (booking даёт одному пользователю одно место на
событие), но их достаточно только выпустить токенами, без записей в `auth`.
