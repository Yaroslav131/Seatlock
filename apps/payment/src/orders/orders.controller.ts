...
export class OrdersController {
  ...
  @ApiOperation({
    summary:
      'Создать заказ на место, которое сейчас держит вызывающий (переносит холд booking в персистентный заказ). Идемпотентно: повторный запрос того же пользователя на то же место возвращает уже созданный заказ',
  })
  @ApiBearerAuth()
